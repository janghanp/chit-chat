...
const PrivateChatRoom = ({ privateChatRoom, setIsSidebarOpen }: Props) => {
	...
	if (hasMessage) {
		const gap = new Date().getTime() - new Date(privateChatRoom.messages![0].createdAt).getTime();
		// 86400000 = 24 hours
	...
	return (
		<tr
			className={`hover:cursor-pointer ${chatId === privateChatRoom.id ? 'active' : ''} w-full`}
				...
				<div className="flex w-full items-center justify-start gap-x-3">
					...
					<div className="flex w-full flex-col">
							...
							<span>
								<time className="ml-2 text-xs opacity-50">
									{!hasMessage ? (
										...
										<>
											{isToday && hasMessage ? (
												<>{format(new Date(privateChatRoom.messages![0].createdAt), 'p')}</>
											) : (
												<>{format(new Date(privateChatRoom.messages![0].createdAt), 'MM/dd')}</>
											)}
										</>
									)}
								</time>
						...
						<div className="max-w-[210px] overflow-x-hidden text-ellipsis text-sm font-normal">
							{hasMessage && (
								<span>
									{privateChatRoom.messages![0].sender.username}: {privateChatRoom.messages![0].text || 'image'}
								</span>
							)}
						</div>
					</div>
